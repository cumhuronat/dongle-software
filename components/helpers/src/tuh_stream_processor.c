...
AccessoryUpdate currentAccessoryUpdate;
...
void parse_status(char *statusString)
{
    ...
    for (int i = 0; i < segmentCount; i++)
    {
        ...
        {
            PinUpdate pinUpdate = {};
            pinUpdate.p = strstr(segment + 1, "P") != NULL;
            ...
            if (pinUpdate.p != currentPinUpdate.p || pinUpdate.x != currentPinUpdate.x || pinUpdate.y1 != currentPinUpdate.y1 || pinUpdate.y2 != currentPinUpdate.y2 || pinUpdate.z != currentPinUpdate.z || pinUpdate.h != currentPinUpdate.h)
            {
                updateMessage.type = PIN_UPDATE;
                updateMessage.data.pinUpdate = pinUpdate;
                xQueueSend(xQueueUpdates, &updateMessage, 0);
                currentPinUpdate = pinUpdate;
                //ESP_LOGI(TAG, "Pin: P=%d X=%d Y1=%d Y2=%d Z=%d H=%d", pinUpdate.p, pinUpdate.x, pinUpdate.y1, pinUpdate.y2, pinUpdate.z, pinUpdate.h);
            }
        }
        ...
        else if (strncmp(segment, "Ov:", 3) == 0)
        {
            ...
            if (i < (segmentCount - 1) && strncmp(segments[i + 1], "A:", 2) == 0)
            {
                accessoryUpdate.coolant = strstr(segments[i + 1], "F") != NULL;
            }
            else
            {
                accessoryUpdate.coolant = false;
            }

            if (accessoryUpdate.coolant != currentAccessoryUpdate.coolant)
            {
                updateMessage.type = ACCESSORY_ITEMS_UPDATE;
                updateMessage.data.accessoryUpdate = accessoryUpdate;
                xQueueSend(xQueueUpdates, &updateMessage, 0);
                currentAccessoryUpdate = accessoryUpdate;
                //ESP_LOGI(TAG, "AccessoryItems: C:%d", accessoryUpdate.coolant);
            }
        }
    }
}