...
bool hcd_init(uint8_t rhport)
{
    ...
    ESP_LOGI("MAX3471", "Revision: 0x%02X", revision);
...
static void handle_xfer_done(uint8_t rhport, bool in_isr)
{
    ...
    debugLog("xfer_done %d %d %d", ep_num, ep_dir, hresult);
    ...
    case HRSL_NAK:
        ...
        if (ep == next_ep)
        {
            ...
            debugLog("retry %d", ep->hxfr_bm.ep_num);
            hxfr_write(rhport, _hcd_data.hxfr, in_isr);
        }
        else if (next_ep)
        {
            ...
            debugLog("next_ep %d", next_ep->hxfr_bm.ep_num);
            xact_generic(rhport, next_ep, true, in_isr);
        }
        else
        {
